...
model_name = 'mnist_model_'+version
...
size_train, img_rows_x, img_cols_x = x_train.shape
size_test, img_rows_x, img_cols_x = x_test.shape

x_train = np.reshape(x_train,(size_train,1,img_rows_x, img_cols_x))
x_test  = np.reshape(x_test,(size_test,1,img_rows_x, img_cols_x))
...
model.add(Convolution2D(256, (3,3), strides = (2,2), padding='same',input_shape=(1,img_rows_x, img_cols_x)))
...
# Fit the model
model.fit(x_train, y_train, validation_data=(x_test,y_test), epochs=100, batch_size=batch_size,shuffle=True, verbose=1)
